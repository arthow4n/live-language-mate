...
import {
  ...
  expectToNotBeNull,
} from '../../__tests__/typedExpectHelpers.js';
...
function createTestFile(options: {
  ...
  const { name, size = 1024, type } = options;
...
function createTestImageFile(name = 'test.jpg', size = 1024): File {
  return createTestFile({ name, size, type: 'image/jpeg' });
}
...
describe('ImageStorage Integration Tests', () => {
  ...
  describe('saveImage', () => {
    ...
    test('should reject unsupported file types', async () => {
      ...
      await expect(imageStorage.saveImage(invalidFile)).rejects.toThrow(
        ImageStorageError
      );
      await expect(imageStorage.saveImage(invalidFile)).rejects.toThrow(
  ...
  describe('getImage', () => {
    ...
    test('should preserve file content integrity', async () => {
      ...
      const retrievedContent = new Uint8Array(
        await retrievedFile.arrayBuffer()
      );
  ...
  describe('listImages', () => {
    ...
    test('should list all saved images', async () => {
      ...
      const file2 = createTestFile({ name: 'test2.png', type: 'image/png' });
