...
import { supportedImageMimeTypes } from '../schemas/imageAttachment.js';
import { imageStorage } from '../services/imageStorage.js';
import { convertToBase64DataURL, validateImageFile } from '../services/imageUtils.js';
...
export function useImageUpload(options: UseImageUploadOptions = {}): UseImageUploadReturn {
  ...
  const updateImageItem = React.useCallback((imageId: string, updates: Partial<ImageUploadItem>) => {
  ...
  const processImageFile = React.useCallback(async (file: File): Promise<ImageUploadItem | null> => {
    try {
      // Validate the file
  ...
  const uploadImages = React.useCallback(async (files: File[]): Promise<void> => {
    if (files.length === 0) return;

    // Check max images limit
    if (maxImages && images.length + files.length > maxImages) {
      if (onError) {
        onError(`Cannot upload ${String(files.length)} images. Maximum ${String(maxImages)} images allowed.`);
      }
      return;
    }
    ...
    try {
      ...
      const placeholderItems: ImageUploadItem[] = files.map(file => ({
        ...
        image: {
          ...
          // eslint-disable-next-line @typescript-eslint/consistent-type-assertions -- Safe type assertion for placeholder
          mimeType: supportedImageMimeTypes.includes(file.type as never) 
            ? (file.type as never) 
            : 'image/png', // fallback for placeholder
          savedAt: new Date(),
          size: file.size,
        },
        isLoading: true,
      ...
      const processedItems = await Promise.all(
        files.map(async (file, index) => {
          ...
          if (!result) {
            ...
            setImages(prev => prev.filter(item => item.image.id !== placeholderId));
            return null;
          }
      ...
      const successfulItems = processedItems.filter((item): item is ImageUploadItem => item !== null);
  ...
  const removeImage = React.useCallback(async (imageId: string) => {
    try {
  ...
  const retryImage = React.useCallback(async (imageId: string) => {
